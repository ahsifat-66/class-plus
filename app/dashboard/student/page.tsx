...
function StudentDashboardContent() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <Navbar onJoinClassOpen={() => setIsJoinOpen(true)} />
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="rounded-3xl bg-gradient-to-r from-emerald-950 via-teal-950 to-slate-900 p-6 sm:p-8 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-semibold text-emerald-200 backdrop-blur-md border border-emerald-400/20">
                ...
                <span>Student Action Center • Authorized STUDENT</span>
              </div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
                Welcome, {currentUser?.name || "Student"}
              </h1>
              <p className="text-sm text-emerald-200 max-w-2xl leading-relaxed">
                Stay on top of upcoming assignment deadlines, collaborate with classmates in discussion channels, and ask the Socratic AI Tutor for guided discovery.
              </p>
            </div>
          ...
          <div className="mt-6 pt-5 border-t border-white/10 grid grid-cols-3 gap-4 max-w-md">
            <div>
              <span className="text-xs text-emerald-300 block">Enrolled Classes</span>
              <span className="text-xl font-bold text-white">{classrooms.length}</span>
            </div>
            <div>
              <span className="text-xs text-emerald-300 block">Pending Deliverables</span>
              <span className="text-xl font-bold text-white">
                {dueSoonTasks.filter((t) => !t.submission).length}
              </span>
            </div>
            <div>
              <span className="text-xs text-emerald-300 block">Role Verification</span>
              <span className="text-xs font-bold text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded-full inline-block mt-1">
                Active Student
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Upcoming deadlines across your enrolled classes
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {dueSoonTasks.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="flex flex-col justify-between rounded-2xl border border-slate-200 p-4 bg-slate-50/50 hover:bg-white hover:border-emerald-300 hover:shadow-md transition-all group"
                  >
                    <div>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-200/60 flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 hover:text-emerald-900 group-hover:translate-x-0.5 transition-all"
                      >
          ...
          {isLoadingClasses ? (
            ...
            <div className="rounded-3xl border-2 border-dashed border-slate-200 bg-white p-12 text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                Ask your instructor for their 6-character class code (e.g. <span className="font-mono font-bold">DBMS45</span>) and join immediately.
              </p>
              <div>
                <button
                  onClick={() => setIsJoinOpen(true)}
                ...
                >
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                    <span className="text-[11px] text-slate-400">
                      {cls.announcements?.length || 0} update
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-slate-50 flex items-center justify-center">
          <div className="text-xs text-slate-400">Loading Student Portal...</div>
        </div>
      }
    >
