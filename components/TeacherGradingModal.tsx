...
export default function TeacherGradingModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-5xl h-[92vh] max-h-[900px] flex flex-col rounded-3xl border border-slate-200 bg-white shadow-2xl animate-in zoom-in-95 duration-150 overflow-hidden">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-slate-100 bg-slate-50/80 shrink-0">
          ...
          <button
            onClick={onClose}
            className="rounded-xl p-2 text-slate-400 hover:bg-slate-200/60 hover:text-slate-600 transition-colors shrink-0"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {submissions.length > 0 && (
          <div className="md:hidden border-b border-slate-100 bg-slate-50/50 p-2 overflow-x-auto shrink-0 flex items-center gap-2 scrollbar-none">
            {submissions.map((sub, idx) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all shrink-0 ${
                    isSelected
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="hidden md:flex flex-col w-72 border-r border-slate-200 bg-slate-50/40 shrink-0 overflow-y-auto">
            <div className="p-3 border-b border-slate-200/60 flex items-center justify-between text-xs font-bold text-slate-500 uppercase tracking-wider">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-slate-100 flex-wrap gap-3">
                  ...
                  <div className="flex items-center gap-1.5 self-end sm:self-auto">
                    <button
                      ...
                      className="p-1.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:pointer-events-none transition-colors"
                ...
                <div className="space-y-4">
                  ...
                  {parsedContent.driveUrl && (
                    <div className="rounded-2xl border-2 border-indigo-200 bg-gradient-to-r from-indigo-50/70 to-blue-50/50 p-4 shadow-sm hover:border-indigo-400 transition-all">
                      <div className="flex items-center justify-between flex-wrap gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="h-10 w-10 rounded-xl bg-white shadow-sm flex items-center justify-center shrink-0 border border-indigo-100">
                          ...
                          <div className="min-w-0">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-700 bg-indigo-100/70 px-2 py-0.5 rounded-full inline-block mb-0.5">
                              Google Drive Project File
                            </span>
                            <div className="text-xs sm:text-sm font-semibold text-slate-900 truncate max-w-md">
                              {parsedContent.driveUrl}
                            </div>
                          </div>
                        </div>
                        ...
                        <a
                          ...
                          className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs sm:text-sm font-bold text-white shadow-sm hover:bg-indigo-700 transition-all shrink-0"
                        >
                  ...
                  {parsedContent.fileUrl && (
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 space-y-3">
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1.5 rounded-xl bg-white border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors shadow-sm"
                          >
                      ...
                      {(parsedContent.fileType?.startsWith("image/") ||
                        ...
                        <div className="relative group rounded-xl overflow-hidden border border-slate-200 bg-black/5 max-h-72 flex items-center justify-center">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                          ...
                          <button
                            type="button"
                            onClick={() => setZoomedImage(parsedContent.fileUrl || null)}
                            className="absolute bottom-2 right-2 rounded-lg bg-slate-900/70 hover:bg-slate-900 text-white p-2 backdrop-blur-sm transition-all"
                            title="Zoom Image"
                          >
                  ...
                  {parsedContent.text ? (
                    <div className="space-y-1.5">
                      ...
                      <pre className="rounded-2xl bg-slate-900 p-4 text-xs font-mono text-emerald-400 overflow-x-auto whitespace-pre-wrap leading-relaxed shadow-inner max-h-64">
                        {parsedContent.text}
                      </pre>
                    </div>
                  ) : !parsedContent.driveUrl && !parsedContent.fileUrl ? (
                    <div className="rounded-2xl bg-slate-50 p-4 text-center text-xs text-slate-400 border border-slate-200">
                ...
                <form onSubmit={(e) => handleSaveGrade(e, false)} className="space-y-4 pt-3 border-t border-slate-200">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative w-36 sm:w-44">
                        <input
                          ...
                          className="w-full text-base sm:text-sm rounded-xl border border-slate-300 px-3.5 py-2.5 font-bold text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                          required
                        />
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <button
                          ...
                          className="rounded-lg bg-slate-100 hover:bg-indigo-100 hover:text-indigo-700 px-2.5 py-1.5 text-xs font-bold text-slate-700 transition-colors"
                        >
                          100%
                        </button>
                        <button
                          type="button"
                          onClick={() => handleQuickScore(90)}
                          className="rounded-lg bg-slate-100 hover:bg-indigo-100 hover:text-indigo-700 px-2.5 py-1.5 text-xs font-bold text-slate-700 transition-colors"
                        >
                          90%
                        </button>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] text-slate-400">Student will see this</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap pb-1">
                      <button
                        ...
                        className="rounded-full bg-slate-100 hover:bg-slate-200 px-2.5 py-1 text-[11px] font-medium text-slate-600 transition-colors"
                      >
                        + Great work!
                      </button>
                      <button
                    ...
                    <textarea
                      ...
                      className="w-full text-base sm:text-xs rounded-2xl border border-slate-300 p-3 text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 leading-relaxed"
                    />
                  </div>

                  {successMsg && (
                    <div className="flex items-center gap-2 text-xs sm:text-sm text-emerald-700 bg-emerald-50 p-3 rounded-xl border border-emerald-200">
                  ...
                  {errorMsg && (
                    <div className="text-xs sm:text-sm text-rose-700 bg-rose-50 p-3 rounded-xl border border-rose-200">
                      {errorMsg}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2 flex-wrap">
                    {selectedIndex < submissions.length - 1 && (
                      <button
                        ...
                        className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-xs sm:text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 transition-all disabled:opacity-50"
                      >
                        Save & Next Student
                      </button>
                    )}
                    <button
                      type="submit"
                      disabled={isSaving}
                      className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-xs sm:text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 transition-all disabled:opacity-50"
                    >
